...
class SyRFD:
    ...
    def _get_safe_fallback_value(self, attr, use_decimal=False):
        ...
        • LHS-only / both: generate a value guaranteed to be *far* (distance
          > threshold) from every existing value, but bounded by the actual
          data range + a small buffer.  This makes the LHS condition false for
          all existing tuples, so no RFD can be triggered.
        ...
        • Free (no dependency): uniform sample over the data range.
        """
        ...
        if role == 'free':
            val = random.uniform(data_min, data_max)
            print(f"  {attr} [free fallback]: uniform sample -> {val:.3f}")
            if attr in self.integer_attrs:
                return int(round(val))
            return round(val, 2)

        # LHS-only or both: produce a value with distance > threshold from
        # every existing value.  We try both sides of the distribution and
        # add a random jitter in [0, threshold] for diversity.
    ...
    def _get_rhs_constrained_value(self, rhs_attr, generated_values, use_decimal=False):
        ...
        if not constraint_applied:
            # No existing tuple has similar LHS → sample freely
            mean = stats.get('mean', (data_min + data_max) / 2)
            std  = stats.get('std', 1.0)
            val  = np.clip(np.random.normal(mean, std * 0.4), data_min, data_max)
            print(f"  {rhs_attr} [RHS constrained, no similar LHS]: free sample -> {val:.3f}")
