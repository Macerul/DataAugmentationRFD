...
def main():
    # Parameters
    dataset_dir= 'imbalance_datasets_CasTGAN/generated_data'
    dataset_files = sorted(glob.glob(os.path.join(dataset_dir, '*_train.csv')))
    print("NUM DATASETS: ", len(dataset_files))

    cm_dir = 'classification_results_casTGAN'
    new_tuples_dir = 'classification_results_casTGAN/new_tuples'
    os.makedirs(cm_dir, exist_ok=True)
    os.makedirs(new_tuples_dir, exist_ok=True)

    for data_path in dataset_files:
        print('PROCESSING:', data_path)
        basename = os.path.basename(data_path)
        dname = os.path.splitext(basename)[0].split('_train')[0]
        target_col = 'class'
        random_state = 42
        ...
        print("Required training samples:", required_train_samples)

        df_generated = pd.read_csv(f'imbalance_datasets_CasTGAN/generated_data/{dname}_train.csv')
        minority_generated = df_generated[df_generated[target_col] == min_class]
        if len(minority_generated) >= required_train_samples:
            casTGAN_tuples = minority_generated.sample(required_train_samples, random_state=random_state)
            print(f"Campionate {required_train_samples} tuple sintetiche dalla classe {min_class}.")
        else:
            casTGAN_tuples = minority_generated
            print(f"Disponibili solo {len(minority_generated)} tuple sintetiche per la classe {min_class}")

        synthetic_csv_path = os.path.join(new_tuples_dir, f"{dname}_new_tuples_casTGAN.csv")
        casTGAN_tuples.to_csv(synthetic_csv_path, index=False)
        print('castgan tuples:\n', casTGAN_tuples.head())


        X_train_resampled = pd.concat([df,casTGAN_tuples], ignore_index=True)
        X_train = X_train_resampled.drop(target_col, axis=1)
        y_train = X_train_resampled[target_col]
        ...
        test_dir = 'imbalance_datasets_CasTGAN/test'
        test_csv_path = os.path.join(test_dir, f"{dname}_test.csv")
        print(f'LOADING {test_csv_path}')
        X_test_ctgan = load_data(test_csv_path)
        X_test = X_test_ctgan.drop(target_col, axis=1)
        print('X_test:', X_test.head())
        y_test = X_test_ctgan[target_col]
        ...
        for name, model_config in models_params.items():
            ...
            plot_confusion_matrix(
                ...
                filename=os.path.join(cm_dir, f"{name.replace(' ', '_')}_cm_{dname}_aug_gridsearch_casTGAN.pdf")
            )
        ...
        results_csv = f'{dname}_classification_results_aug_gridsearch_casTGAN.csv'
        ...
        params_csv = f'{dname}_best_parameters_gridsearch_casTGAN.csv'
