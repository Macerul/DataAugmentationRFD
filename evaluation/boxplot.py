...
def boxplot_metrics_per_dataset(df=None, figsize_per_subplot=(4,3)):
    ...
    # Parametri modificabili
    # ==========================
    csv_file = 'aggregated.csv'  # path al CSV
    plots_per_row = 6             # numero di plot per riga
    plot_width = 2                # larghezza di ogni subplot
    plot_height = 2               # altezza di ogni subplot
    ...
    datasets_to_plot = ['$D_{13}$', '$D_{24}$', '$D_{28}$']
    ...
    # Crea figura
    fig, axes = plt.subplots(rows, cols, figsize=(cols * plot_width, rows * plot_height),
    ...
    for dataset in ordered_datasets:
        ...
        for metric in metrics:
            ...
            # Boxplot
            sns.boxplot(x='method', y=metric, data=df_dataset, ax=ax, palette="Set2", boxprops=dict(facecolor='none', edgecolor='black', linewidth=0.5),
                        whiskerprops=dict(color='black', linewidth=0.5),
                        capprops=dict(color='black', linewidth=0.5),
                        medianprops=dict(color='black', linewidth=0.8),
                        flierprops=dict(marker='.', color='black', markersize=2, alpha=0.9))
            ...
            for k, method in enumerate(methods):
                ...
                for l, row in df_subset.iterrows():
                    print(row["model"],model_markers[row["model"]],model_colors[row["model"]])
            ...
            # Titolo e label
            ax.set_title(f"{dataset} - {metric}", fontsize=8)
            ax.set_xlabel("")
